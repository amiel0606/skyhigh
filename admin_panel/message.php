...
<script>
    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        if (msg.isFromAdmin) {
                            messageContainer.innerHTML = `
                                <p class="my-message is-primary">
                                    <strong class="has-text-white">Admin:</strong> ${msg.message}
                                    <small class="has-text-grey-lighter">${formatTimestamp(msg.timestamp)}</small>
                                </p>`;
                        } else {
                            messageContainer.innerHTML = `
                                <p class="contact-message">
                                    <strong class="has-text-white">User:</strong> ${msg.message}
        ...
        function sendMessage() {
            ...
            const messageContainer = document.createElement('div');
            messageContainer.className = 'message-container';
            messageContainer.innerHTML = `<p class="my-message is-primary"><strong class="has-text-white">Admin:</strong> ${message} <small class="has-text-grey-lighter">${formatTimestamp(new Date())}</small></p>`;
            chatMessages.appendChild(messageContainer);
            chatMessages.scrollTop = chatMessages.scrollHeight;
            
            messageInput.value = '';
            
            axios.post('./controller/adminMessaging.php', {
                message: message,
